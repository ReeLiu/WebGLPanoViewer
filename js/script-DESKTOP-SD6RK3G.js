...
function update() {
    ...
    if(delta_d_focal < 0)
    {
        uni_d_focal += delta_d_focal;
        if (uni_d_focal > 1)
            uni_d_focal -= 1;
    }

    else if (delta_d_focal > 0)
    {
        uni_d_focal -= delta_d_focal;
        if (uni_d_focal < 0)
            uni_d_focal += 1;
    ...
    house.skyBox.material.uniforms.refocus = refocus;
    house.skyBox.material.needsUpdate = true;
    delta_d_focal = 0;
